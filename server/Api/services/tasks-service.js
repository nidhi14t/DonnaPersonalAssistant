...
function decide(data, routine) {
  ...
  for (
    let i = classSchedule.startTime;
    i < classSchedule.duration + classSchedule.startTime;
    i++
  ) {
    dayArray[i] = 1;
  }
  ...
  for (
    let i = busyTime.startTime;
    i < busyTime.duration + busyTime.startTime;
    i++
  ) {
    dayArray[i] = 1;
  }
  ...
  while (dayFreeHours > 1) {
    for (let i = 0; i < data.length; i++) {
      if (data[i].timeTaking <= dayFreeHours) {
        ...
        let startTimeFormat = moment(startTimeFor).format(
          "YYYY-MM-DDTHH:mm:ss"
        );
